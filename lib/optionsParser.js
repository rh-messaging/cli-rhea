...
class ConnectionOptions {
    ...
    parseConnectionOptions(listArgs) {
        ...
        this.useConfigFile = this.castBoolProperty(args['conn-use-config-file']);
    ...
    */
    ...
    parseUrlString(urlString) {
        ...
        if(regex.test(add)) {
            splitAdd = add.split('@');
            parsedData.username = decodeURIComponent(splitAdd[0].split(':')[0] ? splitAdd[0].split(':')[0] : '');
            parsedData.password = decodeURIComponent(splitAdd[0].split(':')[1] ? splitAdd[0].split(':')[1] : '');
