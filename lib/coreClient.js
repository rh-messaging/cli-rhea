...
class CoreClient {
     ...
     */
    close(context, closeSleep, isListener) {
        if(closeSleep) {
            this.timeoutClose(context, closeSleep, isListener);
        }else{
            if (CoreClient.logStats === 'endpoints') {
                Utils.PrintStatistic(context);
            }

            clearTimeout(this.timer_task);
     ...
     * @param {boolean} isListener - true if container running as listener
     */
    timeoutClose(context, timeout, isListener) {
        if(timeout > 0) {
            this.timeout = timeout;
            this.timeoutFunction = setTimeout(function(context, isListener) {
                this.close(context, null, isListener);
            }, timeout,
            context, isListener);
        }
    }
     ...
     */
    resetTimeout(context, isListener) {
        this.cancelTimeout();
        this.timeoutClose(context, this.timeout, isListener);
    }
    ...
    _buildFailoverHandler() {
        this.arrHosts.push(this.options.url);
        this.arrPorts.push(this.options.port);

        try{
            const connUrls = JSON.parse(this.options.connUrls);
            for(let i = 0; i < connUrls.length; i++) {
                const splitUrl = connUrls[i].split(':');
                this.arrHosts.push(splitUrl[0]);
                this.arrPorts.push(splitUrl[1]);
            }

            return function() {
                return {host: this.arrHosts[this.reconnectCount % this.arrHosts.length],
                    port: this.arrPorts[this.reconnectCount % this.arrPorts.length]};
            };
        }catch(err) {
            throw new ErrorHandler('conn-urls has wrong format, try help fo show how use it');
        }
    }
    ...
    buildReceiverOptionsDict() {
        ...
        if (this.options.action !== 'acknowledge') {
            receiverOptions.autoaccept = false;
        }
        receiverOptions.credit_window = (this.options.recvBrowse || this.options.count === 0 || this.options.duration > 0) ? 0 : undefined; //disable automatic credit windows for recv browse or read all messages from queue
