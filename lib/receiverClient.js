    ...
    _setUpHandlers() {
        ...
        this.container.on('message', function(context) {
            ...
            if(self.received <= self.expected || self.expected === 0) {
                ...
                if(self.options.action === 'acknowledge') {
                    context.delivery.accept();
                }else if(self.options.action === 'reject') {
        ...
        try{
            if(this.options.websocket) {
                ...
                if(!this.options.recvListen) {
                    ...
                    this.container.connect(connectionParams)
                        .open_receiver(this.buildReceiverOptionsDict());
                }else {
                    //run local listener
                    this.server = this.container.listen({port: this.options.recvListenPort});
                    if(this.options.timeout > 0) {
                        this.timeoutClose(null, this.options.timeout);
                    }
                }
            }
        }catch(err) {
